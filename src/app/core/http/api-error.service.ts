...
export class ApiErrorService {
  ...
  private getStatusMessage(statusCode: number, fallbackMessage: string): string {
    switch (statusCode) {
      case 0:
        return 'No fue posible conectar con el servidor. Verifica que la API esté disponible.';
      case 401:
  ...
  private translateMessage(message: string): string {
    ...
    const translations: Record<string, string> = {
      ...
      'Ya existe un check-in registrado para el día actual.':
        'Ya registraste tu entrada para el día operativo actual.',
      'Ya existe un check-out registrado para el día actual.':
        'Ya registraste tu salida para el día operativo actual.',
      'Only pending vacation requests can perform this action.':
        'Solo las solicitudes pendientes pueden realizar esta acción.',
      'The authenticated user is not linked to an active employee.':
        'Tu cuenta no está vinculada a un empleado activo.',
      'The authenticated user is not linked to a valid employee.':
        'Tu cuenta no está vinculada a un empleado válido para este portal.',
      'No open attendance record was found for the current employee.':
        'No se encontró una asistencia abierta para el empleado actual.',
      ...
      'Vacation requests are disabled for the current company.':
        'Las solicitudes de vacaciones están deshabilitadas para la empresa actual.',
      'Request completed successfully.': 'La solicitud se completó correctamente.',
      ...
      'La empresa requiere ubicación GPS para registrar asistencia.':
        'Tu empresa requiere ubicación GPS para registrar asistencia.',
      'La ubicación del registro está fuera del radio permitido de la sucursal.':
        'Tu ubicación está fuera del radio permitido de la sucursal.'
    };
